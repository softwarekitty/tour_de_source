...
def extract_flags(regexFunction, regex_citation, logger):
    if regexFunction == 0:
        ...
        logger.error("PyReS - extract_flags, cannot extract flags from " + str(regex_citation))
        return 0


# (uniqueSourceID int, sourceJSON text, scanID int, fileHash char(44), filePath text, pattern text, flags int, regexFunction int)
def record_regex_citation(uniqueSourceID, sourceJSON, fileHash, pythonFilePath, pattern, flags, regexFunction, report_db, logger):
    logger.debug("PyReS - record_regex, uniqueSourceID: " + str(uniqueSourceID) + " sourceJSON: " + str(sourceJSON) + " fileHash: " + str(fileHash) + " filePath: " + str(pythonFilePath) + " pattern: " + str(pattern) + " flags: " + str(flags) + " regexFunction: " + str(regexFunction))
...
def extractRegexR(child, report_db, uniqueSourceID, sourceJSON, fileHash, pythonFilePath, citationSet, endFileScanS, target_func, logger):
    ...
    if isinstance(child, node_classes.CallFunc) and child.func.as_string() in target_func:
        ...
        for x in child.args:
            if isinstance(x, node_classes.Getattr):
                ...
                if isinstance(gc, scoped_nodes.Module):
                    ...
                    logger.warning("PyReS - extractRegexR, unexpected class found: " + x.__class__.__name__)
        ...
        flags = extract_flags(regexFunction, regex_citation, logger)
        ...
        if citationTuple not in citationSet:
            ...
            record_regex_citation(uniqueSourceID, sourceJSON, fileHash, pythonFilePath, pattern, flags, regexFunction, report_db, logger)
    for grandchild in child.get_children():
        extractRegexR(grandchild, report_db, uniqueSourceID, sourceJSON, fileHash, pythonFilePath, citationSet, endFileScanS, target_func, logger)


def scanFile(pythonFilePath, report_db, uniqueSourceID, sourceJSON, fileHash, cleanFilePath, citationSet, endFileScanS, logger):
    target_func = ["re.compile", "re.search", "re.match", "re.split", "re.findall", "re.finditer", "re.sub", "re.subn"]
    node = astroid.manager.AstroidManager().ast_from_file(pythonFilePath)
    extractRegexR(node, report_db, uniqueSourceID, sourceJSON, fileHash, cleanFilePath, citationSet, endFileScanS, target_func, logger)
        ...
        for pythonFilePath in pythonPaths:
            try:
                ...
                if fileHash not in shaSet:
                    ...
                    memBefore = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
                    leakyWorker = mp.Process(target=scanFile(pythonFilePath, report_db, uniqueSourceID, sourceJSON, fileHash, cleanFilePath, citationSet, endFileScanS, self.logger))
                    leakyWorker.start()
                    leakyWorker.join()
                    memAfter = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
                    progressCounter = 2
                    if memAfter - 1024 > memBefore:
                        self.logger.warning("memory has grown by at least 1K after joining leakyWorker, memAfter: " + str(memAfter))
                else:
                    nDuplicates += 1
