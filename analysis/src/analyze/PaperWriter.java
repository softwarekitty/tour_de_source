...
//		filesToMake.add(new NameContentsPair("topNW.tex", Composer.composeRankTable(10, corpus.iterator(), 2.4, "pattern", "nProjects", "Top 10 Patterns by nProjects (RQ1)", "table:topNW")));
	...
	private static String exportPatternWeightMap(
		...
		for (WeightRankedRegex wrr : corpus2) {
			String rexShortened = wrr.getContent().replaceAll("\\\\\\\\", "\\\\");
			sb.append(rexShortened + "\t" + wrr.getContent() + "\t" +
				wrr.getRankableValue() + "\n");
		}
		return sb.toString();
	}
	...
	public static String exportCorpusRex(ArrayList<WeightRankedRegex> corpusList) {
		StringBuilder sb = new StringBuilder();
	...
	// database
	public static String exportCorpusRaw(ArrayList<WeightRankedRegex> corpusList) {
		StringBuilder sb = new StringBuilder();
		Iterator<WeightRankedRegex> it = corpusList.iterator();
		int i = 0;
		while (it.hasNext()) {
			WeightRankedRegex wrr = it.next();
			sb.append(i + "\t" + wrr.getRankableValue() + "\t" +
				wrr.getContent() + "\n");
			i++;
		}
		return sb.toString();
