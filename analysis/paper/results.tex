...
Since Rex does not support all the features present in the corpus, and because we discarded patterns found in only one project, we only generated sets of matching strings for 2,871 (21\%) of the \DTLfetch{data}{key}{nCorpus}{value} patterns in the corpus. The impact is that 923 projects were excluded from the data set for the similarity analysis. Omitted features are indicated in Table~\ref{table:featureStats}, as described in Section~\ref{results:rq3}.
...
This category contains 17 clusters, each appearing in an average of 15.3 projects, with at least one pattern within this category present in 132 projects. Each of the clusters uses at least one endpoint anchor to require matches to be absolutely positioned, for example:
\verb!`(\w+)$'(35)!, \verb!`^-?\d+$'(17)!, \verb!`^\s'(16)!, and \verb!`^[ -~]*$'(10)!.

\subsubsection{Requiring Two or More Characters In Sequence}
This category contains 18 clusters, each appearing in an average of 12 projects, with at least one pattern within this category present in 124 projects. These clusters require several characters in a row to match some pattern, for example:
\verb!`\d+\.\d+'(30)!, \verb!`  '(17)!, \verb!`([A-Z][a-z]+[A-Z][^ ]+)'(11)!, and \verb!@[a-z]+'(9)!.

\subsubsection{Requiring a Specific Character to Match}
This category contains 18 clusters, each appearing in an average of 19.4 projects, with at least one pattern within this category present in 189 projects. Each cluster requires one specific character to match, for example:
