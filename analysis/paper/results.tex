...
A pattern uses the literal feature to specify matching any specific character.  An example pattern that contains only one literal token is the pattern \verb!`a'!.  This pattern only matches the lowercase letter `a'.  The literal feature was found in \DTLfetch{data}{key}{P_LITERAL_PRESENT}{value}\% of patterns.
...
\subsubsection{The contents of Table~\ref{table:featureStats}}
...
\subsubsection{The 7 Most Rarely Used Features}
...
\subsubsection{General Information About Clusters Found}
 From 9503 distinct patterns, our clustering technique identified 514 clusters of 2 or more patterns, and 7214 clusters of size 1.
  % Clusters are ordered by first by the number of projects containing any pattern within the cluster, and second by size with larger clusters appearing earlier in the list.
...
\subsubsection{Clusters Based On Single Literal Characters}
...
\verb!`<.+>'(63)!, \verb!`<([^>]*)>: \[0x([0-9a-f]+)-0x([0-9a-f]+)\]'(57)!, \verb!`(<[^<>]*)/>'(35)!, \verb!`<!\s+([^<>]*)>'(35)!, \verb!`&(#\d+|#x[0-9a-fA-F]+|\w+);'(25)!, \verb!`&#?\w+;'(24)!, \verb!`<title>[^<]*Revision \d+:'(17)!, \verb!`<(/?)\s*i\s*>'(16)!

\subsubsection{Clusters Based On Matching Whole Lines}
\verb!`^\d+$'(78)!, \verb!`^\w+$'(74)!, \verb!`^\s*$'(59)!, \verb!`^[-\w]+$'(45)!, \verb!`^[a-zA-Z]\w*$'(43)!, \verb!`^[ ]*(#.*)?$'(41)!, \verb!`^[a-f0-9]{40}$'(34)!, \verb!`^[ -~]*$'(25)!, \verb!`^\s*plural$'(19)!
...
\verb!`^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))?'(40)!, \verb!`^#define\s{1,}(\w{1,}_H)\s{1,}<(.*)>'(21)!, \verb!`'()!, \verb!`'()!, \verb!`'()!, \verb!`'()!, \verb!`'()!, \verb!`'()!, \verb!`'()!, \verb!`'()!, ,
...
% Our behavioral clustering technique found 952 clusters over 2727 patterns, with at least one cluster present in 999 of the 1325\todo{fixMEEE} projects that were compatible with Rex.
