...
These clusters match under a variety of conditions by using a character class or a disjunctive \verb!|!. 
For example: 
...
This is in contrast to the survey in Section~\ref{rq1:survey} in which participants reported to very rarely or never use regexes to check for a single character (Table~\ref{tab:regexactivities}).
...
\verb!`(\w+)$'(35)!, \verb!`^-?\d+$'(17)!, \verb!`^\s'(16)!, and \verb!`^[ -~]*$'(10)! each use  \verb!$! and/or \verb!^!.
...
\verb!`\(.*\)'(29)! matches when content is surrounded by parentheses, \verb!`".*"'(25)! matches  when content is surrounded by double quotes, \verb!`<(.+)>'(23)! matches and captures content surrounded by angled brackets, and \verb!`\[.*\]'(22)! matches when content is surrounded by square brackets.
...
\verb!`\d+\.\d+'(30)! requires one or more digits followed by one or more digits, \verb!`  '(17)! requires two spaces in a row, 
%\verb!`([A-Z][a-z]+[A-Z][^ ]+)'(11)!, 
...
\verb!`^https?://'(23)! matches a web address, \verb!`(.+)=(.+)'(9)! matches an assignment statement, and\\ \verb!`\$\{([\w\-]+)\}'(11)! matches and captures content within string interpolation. 
%, and \verb!`SECRET|PASSWORD|PROFANITIES_LIST'(13)!. 
