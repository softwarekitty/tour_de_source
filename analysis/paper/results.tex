...
\todoMid{Maybe we are looking at this and saying: oh there is no clear favorite, that's too bad for us as researchers.  But instead we can say that diversity is alive in the regex community and that maintaining both ways of doing this is important - tool builders should support both ways of doing things with char classes. }
...
Further, since Rex does not support all the regex features present in the corpus, we only generated sets of strings for measuring similarity for 2,871 (21\%) of the \DTLfetch{data}{key}{nCorpus}{value} patterns in the corpus. The impact is that 923 projects were excluded from the data set for the similarity analysis. Omitted features are indicated in Table~\ref{table:featureStats}, as described in Section~\ref{results:rq3}.
...
\todoMid{With multiple matching alternatives, I think it's important to realize and admit freely that our clustering technique will be somewhat eager to cluster these types of things together as similar.  These regexes will match if just one character in a string matches a (sometimes very wide) range of characters like \verb![a-z_]!.  Think about it - this regex will match all strings generated by Rex that have any lowercase characters, even if Rex just randomly inserted those characters.}
