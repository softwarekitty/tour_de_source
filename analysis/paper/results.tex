...
Then, since Rex does not support all the regex features present in the corpus, we were only able to generate sets of strings for measuring similarity for 2,871 (21\%) of the \DTLfetch{data}{key}{nCorpus}{value} patterns in the corpus. The impact is that 923 projects were excluded from the data set for the similarity analysis. Omitted features are indicated in Table~\ref{table:featureStats} for Rex.
...
% For 7 of of 8 patterns in this example cluster, all patterns matched all strings generated by Rex resulting in 100\% similarity scores.  The pattern \verb!`([^:]+):(.*)'! requires one non-colon character before a colon...
