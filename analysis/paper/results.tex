...
Out of the \DTLfetch{data}{key}{percentProjectsUsingRegex}{value}\ projects scanned, \DTLfetch{data}{key}{percentProjectsUsingRegex}{value}\% (\DTLfetch{data}{key}{nProjectsUsingRegex}{value}) contained at least one \emph{utilization}.  For context about how saturated these projects were with regex \emph{utilizations}, we consider how many files the average project scanned contained, how many of those files contained \emph{utilizations}, and how many \emph{utilizations} occurred per file.

\begin{table}[tb]
\centering
\includegraphics[width=\columnwidth]{../analysis_output/contextHistogram}
...
Although  of the projects observed had at least one regex usage, only \DTLfetch{data}{key}{percentFilesUsingRegex}{value}\% (\DTLfetch{data}{key}{nFilesUsingRegex}{value}) of the files observed had at least one regex usage.

\subsection{Usage Frequency of {\tt re} Module Functions and Flags}
\todo{comments, labels on graph below}
