...
This category contains 19 clusters. Each of the clusters center around a single literal character. For example, three of the top clusters in this category are:
...
This category contains 8 clusters. Each of the clusters begins with the STR anchor and ends with the END anchor, requiring the entire input string to match the pattern. For example, three of the top clusters in this category are:
...
\verb!`<.+>'(63)!, \verb•`<!\s+([^<>]*)>'(35)•, and \verb!`(<[^<>]*)/>'(35)!. %,   \verb!`\(.*\)'(31)!, \verb!`<([^>]*)>: \[0x([0-9a-f]+)-0x([0-9a-f]+)\]'(57)!
%removed long pattern to fit paper columns
.
