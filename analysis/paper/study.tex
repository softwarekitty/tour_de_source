...
At a high level, our semantic analysis clusters regular expressions by their behavioral similarity. We will briefly illustrate our definition of behavioral similarity using some unspecified patterns A and B.  Consider a set `mA' of 100 strings thazt pattern A matches.  If pattern B matches 90 of the 100 strings in the set mA, then B is 90\% similar to A.  Now consider another set of 100 strings, `mB' that pattern B matches.  If pattern A only matches 50 of the strings in `mB', then A is 50\% similar to B.  We use similarity scores to create a similarity matrix as shown in Figure~\ref{fig:minimalMatrix}.  In row A, column B we see that B is 90\% similar to A.  Int row B, column A, we see that A is 50\% similar to B.  Each pattern is always 100\% similar to itself, since it matches all 100 of its own strings by their definition.
