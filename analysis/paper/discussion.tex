...
\todoMid{moved from results...This reinforces the idea of finding flags in text.  This is handled well for ASCII, but in the future one must consider the users who are using different flags/delimiter symbols from other languages.  Are they able to perform this absolutely critical and fundamental feature?  This is related to a phenomenon called `Text Segmentation'.  The Unicode Standard Annex is devoted to managing text segmentation across languages.  It mentions a large number of boundary characters that should be of interest to support, including characters that are exotic to American researchers like `U+060D' the Arabic Date Separator, or `U+18806' the Mongilian Soft Hyphen.}
