...
As mentioned in Section~\ref{rq4:results}, capturing values assigned to variables when parsing source code was one of the main categories of clusters observed.  \todoMid{update?}
...
Delimiters that separate items on one line like \verb!','! are also quite common.  Although survey participants indicated an average frequency of 1.7 (very rarely or never) for ``checking for a single character," we found 19 clusters whose essential behavior was to search for one or two characters.  \todoMid{update!} 
...
\todoMid{update based on clusters}
