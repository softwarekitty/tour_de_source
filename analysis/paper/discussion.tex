...
The features NCCC, RNG, WSP, DEC, WRD, NWSP, NDEC, NWRD are all included as the second priority because by implementing CCC, these features are available as specific instances of CCC.  For example, considering features \todo{X and Y},  $\backslash d \equiv [0123456789]$. Also, considering features \todo{Z and W}, $[a-f] \equiv [abcdef]$. 
Similarly, the features ADD, KLE, QST, SNG and LWB are available once the DBB feature is implemented.  For example, $a* \equiv a\{0, MAX\}$, $a\{5\} \equiv a\{5, 5\}$ and $a? \equiv a\{0, 1\}$. \todo{modify the example structure as above}
...
\subsubsection{One more example}
\todo{add in one more example here}
...
One surprising result of our clustering is that behaviorally speaking, the negation of the word class was more heavily used (208 projects) than the word class itself (114 projects). Why? \todo{You can't ask 'Why?' in a paper. You can provide a possible explanation}
